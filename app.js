...
(function () {
  ...
  /* ============ 体脂秤数据「文本识别」 ============
   *
   * 用户从体脂秤 App / 健康 App 复制出来的文本，典型格式：
   *   体重：65.9kg
   *   体脂率：18.1%
   *   肌肉率：77.7%
   *   体水分量：36.4kg
   *
   * 设计取舍（承接已下线的截图 OCR 的教训）：
   *   - 只认「标签 + 数值」，绝不猜。文本来源本身是结构化的、没有识别误差，
   *     所以匹配到就该信任；反过来匹配不到就返回 null，**绝不能**用
   *     「全文里第一个落在区间的数」去凑 —— 那条路在 OCR 时期已被证明会把
   *     「身体得分 81」当成体重填进表单。宁可让用户手填一项，也不主动造脏数据。
   *   - 标签刻意**不包含**「肌肉」「脂肪」「水分」这类裸词：秤报告里同时存在
   *     「肌肉率 77.7%」和「肌肉量 51.2kg」，用裸词匹配必然取错单位。
   *   - 同一位置多个标签命中时保留最长的（"体水分量" 压过 "体水分" / "水分"）。
   */
  function parseBodyText(txt) {
    const res = { weight: null, fatRate: null, muscleRate: null, waterMass: null };
    if (!txt || typeof txt !== "string") return res;

    // 全角归一：不同 App 复制出来的冒号/数字/括号可能是全角，不归一就匹配不上
    const s = txt
      .replace(/[：]/g, ":")
      .replace(/[％]/g, "%")
    ...
    // 标签表：只放无歧义的词，顺序无关（代码按出现位置 + 长度自动择优）
    const LABELS = [
      { key: "weight",     names: ["体重", "重量", "bodyweight", "weight"] },
      { key: "fatRate",    names: ["体脂率", "体脂肪率", "脂肪率", "脂率", "体脂", "bodyfat", "body fat", "fat rate", "fat"] },
      { key: "muscleRate", names: ["肌肉率", "肌率", "musclerate", "muscle rate", "muscle"] },
      { key: "waterMass",  names: ["体水分量", "体水份量", "身体水分量", "身体水份量", "水分量", "水份量", "体水分", "体水份", "水分", "水份", "bodywater", "body water", "water"] },
    ];
    // 合理区间：只用来剔除明显错配（单位搞错、标签串味），不做精确校验
    const RANGE = {
      weight: [20, 300],
      fatRate: [2, 70],
      muscleRate: [20, 99],
    ...
    // 1) 收集所有标签出现位置
    const hits = [];
    for (const grp of LABELS) {
      for (const n of grp.names) {
        const needle = n.toLowerCase();
        let from = 0, idx;
        while ((idx = LOWER.indexOf(needle, from)) !== -1) {
          hits.push({ key: grp.key, start: idx, end: idx + needle.length });
          from = idx + needle.length; // 同一个标签自身不重叠重复匹配
        }
      }
    }
    if (!hits.length) return res;

    // 2) 位置升序；同一起点则长标签优先，重叠时保留更长的那个
    ...
    // 3) 每个标签的取值区间 = 它之后到下一个标签之前，避免跨标签抓到别人的数
    for (let i = 0; i < kept.length; i++) {
      const h = kept[i];
      const region = s.slice(h.end, i + 1 < kept.length ? kept[i + 1].start : s.length);
      const m = region.match(/(\d+(?:\.\d+)?)\s*(kg|公斤|斤|g|%)?/i);
      if (!m) continue;
      let v = parseFloat(m[1]);
      if (!isFinite(v)) continue;
      const u = (m[2] || "").toLowerCase();
      if (u === "斤") v = v / 2;                        // 市斤 → kg
      else if (u === "g" && v >= 1000) v = v / 1000;    // 36400g → 36.4kg
      const rg = RANGE[h.key];
      if (v < rg[0] || v > rg[1]) continue;             // 越界 = 匹配串味，宁缺勿错
      if (res[h.key] == null) res[h.key] = Math.round(v * 10) / 10; // 重复出现时取第一个
    }
    return res;
  }
  if (typeof window !== "undefined") { window.__parseBodyText = parseBodyText; }
  ...
  function quickWeightModal() {
    openModal("记录体重", `
      ...
      <div class="fld full" style="margin-top:10px;"><label>智能识别</label><textarea id="qwSmart" placeholder="粘贴体脂秤数据自动识别：体重：65.9kg 体脂率：18.1% 肌肉率：77.7% 体水分量：36.4kg"></textarea></div>
      <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
        <button class="btn btn-sm btn-ghost" id="qwPasteBtn">📋 粘贴并识别</button>
        <span id="qwParseHint" class="card-sub" style="flex:1;min-width:120px;"></span>
      </div>
    ...
    // 一键直达：粘贴体脂秤文本 → 解析四个指标 → 填入表单
    const qwPasteBtn = $("#qwPasteBtn"), qwSmart = $("#qwSmart"), qwParseHint = $("#qwParseHint");
    if (qwPasteBtn && qwSmart) {
      const setHint = (s, color) => { if (qwParseHint) { qwParseHint.textContent = s; qwParseHint.style.color = color || "var(--ink-3)"; } };
      const fillBody = (txt) => {
        const r = parseBodyText(txt);
        const ids = { weight: "#qwW", fatRate: "#qwF", muscleRate: "#qwM", waterMass: "#qwWater" };
        let filled = 0;
        for (const k of Object.keys(ids)) {
          const el = $(ids[k]);
          if (r[k] != null && el) { el.value = r[k]; filled++; }
        }
        if (!filled) {
        ...
        const names = { weight: "体重", fatRate: "体脂率", muscleRate: "肌肉率", waterMass: "体水份量" };
        const missing = Object.keys(ids).filter((k) => r[k] == null).map((k) => names[k]);
        const msg = missing.length
          ? `✅ 已识别 ${filled}/4 项，缺 ${missing.join("、")}，可手动补`
          : `✅ 已识别全部 4 项，核对后保存`;
        setHint(msg, missing.length ? "#f59e0b" : "#10b981");
        toast(msg);
      };
      // 一键读剪贴板 → 填入 → 识别；剪贴板被浏览器拒绝时，若已手动粘贴则直接识别现有文本
      qwPasteBtn.onclick = async () => {
        let txt = "";
        try { if (navigator.clipboard && navigator.clipboard.readText) txt = await navigator.clipboard.readText(); } catch (e) { txt = ""; }
        if (txt && txt.trim()) { qwSmart.value = txt; fillBody(txt); }
        else if ((qwSmart.value || "").trim()) { fillBody(qwSmart.value); }
        else { try { qwSmart.focus(); } catch (e) {} toast("无法读取剪贴板，请长按输入框手动粘贴后再点本按钮", "warn"); }
      };
    }
  }
  ...
  function renderWeight() {
    ...
    const wForm = `
      ...
      <div class="fld full" style="margin-top:10px;"><label>智能识别</label><textarea id="wSmart" placeholder="粘贴体脂秤数据自动识别：体重：65.9kg 体脂率：18.1% 肌肉率：77.7% 体水分量：36.4kg"></textarea></div>
      <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
        <button class="btn btn-sm btn-ghost" id="wPasteBtn">📋 粘贴并识别</button>
        <span class="card-sub" id="wParseHint" style="font-size:12px;"></span>
      </div>`;
    ...
    /* ---- 粘贴体脂秤文本 → 解析四个指标 → 填入表单 ---- */
    const wPasteBtn = $("#wPasteBtn"), wSmart = $("#wSmart"), wParseHint = $("#wParseHint");
    if (wPasteBtn && wSmart) {
      const setHint = (s, color) => { if (wParseHint) { wParseHint.textContent = s; wParseHint.style.color = color || "var(--ink-3)"; } };
      const fillBody = (txt) => {
        const r = parseBodyText(txt);
        const ids = { weight: "#wWeight", fatRate: "#wFat", muscleRate: "#wMus", waterMass: "#wWater" };
        let filled = 0;
        for (const k of Object.keys(ids)) {
          const el = $(ids[k]);
          if (r[k] != null && el) { el.value = r[k]; filled++; }
      ...
      // 一键读剪贴板 → 填入 → 识别；剪贴板被浏览器拒绝时，若已手动粘贴则直接识别现有文本
      wPasteBtn.onclick = async () => {
        let txt = "";
        try { if (navigator.clipboard && navigator.clipboard.readText) txt = await navigator.clipboard.readText(); } catch (e) { txt = ""; }
        if (txt && txt.trim()) { wSmart.value = txt; fillBody(txt); }
        else if ((wSmart.value || "").trim()) { fillBody(wSmart.value); }
        else { try { wSmart.focus(); } catch (e) {} toast("无法读取剪贴板，请长按输入框手动粘贴后再点本按钮", "warn"); }
      };
    }
